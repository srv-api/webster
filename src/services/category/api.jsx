...
export const fetchMerkData = async () => {
  try {
    const response = await axiosInstance.get("/merchant/category/merk");
...
export const exportCategorysToExcel = async (from, to) => {
  try {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "categorys_export.xlsx");
    document.body.appendChild(link);
    link.click();
    link.remove();
...
export const fetchCategoryData = async () => {
  try {
    const response = await axiosInstance.get("/merchant/category/category");
    return response.data;
  } catch (error) {
...
export const updateExistingCategory = async (category) => {
  try {
    const response = await axiosInstance.put(`/merchant/category/update/${category.id}`, {
      ...
      minimal_stock: category.minimal_stock,
...
export const fetchCategoryById = async (id) => {
  try {
    const response = await axiosInstance.get(`/merchant/category/${id}`);
    return response.data.data; // Mengembalikan object category
  } catch (error) {
